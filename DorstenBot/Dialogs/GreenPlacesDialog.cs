...
{
    [Serializable]
    public class GreenPlacesDialog : IDialog<string>
    {
   
        public async Task StartAsync(IDialogContext context)
        {
            PromptDialog.Text(context, QuestionEntered, "Du hat also eine Frage zum Thema Grünanlagen. Was willst du wissen?");
        }
        ...
        public virtual async Task QuestionEntered(IDialogContext context, IAwaitable<string> argument)
        {
            ...
            // TODO call QnA
            var answer = Helper.GetAnswerToQuestion(result, Helper.QnAType.GreenPlaces);
        ...
        public virtual async Task QustionAnswered(IDialogContext context, IAwaitable<bool> argument)
        {
            var result = await argument;
            if (result)
                PromptDialog.Confirm(context, RepeatQuestion, "Brauchst du noch etwas zum Thema Grünanlagen?", options: new string[] { "Ja", "Nein" });
            else
        ...
        public virtual async Task RepeatQuestion(IDialogContext context, IAwaitable<bool> argument)
        {
            ...
            if (result)
            {
                ...
                PromptDialog.Text(context, QuestionEntered, "Was willst du noch zum Thema Grünanlagen wissen?");
        }
    }
}
